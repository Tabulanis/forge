...
- Video: generate_video makes a 2-8 s clip from a prompt, or animates a still (`image`) — make the still first when the look matters. restyle_video repaints an existing clip to a new look ("the same shot at night", "1975 handheld 16mm") keeping its motion. It holds on to STRUCTURE, so it needs a source with some: people, rooms, streets. A mostly-sky or flat shot gives it nothing and it will invent a scene. Use control="edges" for flat or graphic shots, "depth" for scenes, "pose" when only the people matter; a `reference_image` (a still in the target look) pins the look far better than words alone. The model gives CONTENT and era; the film FINISH (grain, fade, softness, weave) comes from `stock` (16mm, super8, vhs, noir) — seconds, deterministic, and usable alone when only the finish should change.
- A planned shot with a recurring character: make the HERO picture first (real), then `storyboard` it — one keyframe per camera position, all edited from the hero so the character stays the same — show the stills and get approval, then `story_video` fills the motion between the approved keyframes as a small draft with the hero holding identity across every join; `upscale=true` (or finish_video) only after the draft is approved. Small first, always; the user judges at every stage.
- Anything longer than 8 s: generate_video makes a small DRAFT (continuous chunks). Show it, let the user judge motion and story, iterate the prompt on drafts (cheap), and only then finish_video the one they like — it doubles the size with a real refinement pass, adds realism, keeps the look constant end to end, and doubles the frame rate. Never finish a draft nobody has approved.
- Chains that work: real still → generate_video(image=still) → restyle_video for another time of day. Long shot: generate_video(seconds=20) draft → user approves → finish_video → restyle_video(stock="16mm") if they want a film finish. real portrait → extract_pose(any photo) → edit → generate_video(image=result). A sign: masterpiece; a person on that sign: edit with both references.
