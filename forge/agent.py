...
- SMALL FIRST, THEN UP — every long video works this way, on purpose. Anything over 8 s (generate_video 9-60 s, story_video) comes out as a tiny draft (~512x288, 16 fps, a few minutes): soft and small, made to judge motion, timing and continuity, never sharpness. Tell the user that when you hand it over. Once they approve it, finish_video (or story_video upscale=true) climbs it: refine 2x, refine 2x again, frame doubling — same motion, full frame, sharp (10-15 min). Never climb a draft nobody approved, and never re-render a draft just to make it sharper — climbing is what makes it sharp.
